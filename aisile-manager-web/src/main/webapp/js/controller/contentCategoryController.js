app.controller('contentCategoryController', function($scope,$http,itemCatService,$controller,contentCategoryService,contentService) {
	//继承baseController
	...
	$scope.findSearch = function(page,rows){
		contentCategoryService.findSearch(page,rows,$scope.searchEntity).success(
				function(response){
					$scope.list = response.rows;
					$scope.paginationConf.totalItems = response.total;
				}
